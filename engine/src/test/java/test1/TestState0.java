...
public class TestState0 extends GameState implements EventListener {

  private UIText title;
  private UIText sizeText;
  private UIButton button;
  private UIColorPane colorPane = new UIColorPane(0xFF0000FF);
  private Font font;
  private boolean loaded = false;

  public TestState0(GameWindow window) {
    super(window);
  }
  ...
  public void init() {
    ...
    /*this.title = new UITextClickable(this.font, TEST_TEXT, 16);
    BoundingBox2D boundingBox = this.font.calculateBoundingBox(TEST_TEXT, 16, 500);
    this.title.position(
        new Vector3f(
            this.window.size().x / 2.0f - boundingBox.width() / 2.0f,
            this.window.size().y,
            0));
    this.title.size(new Vector3f(boundingBox.width(), boundingBox.height(), 0));
    ...
    */
    ...
    this.colorPane = new UIColorPane(0xFF8000FF, 0xFF0000FF, 10.f, 10.0f);
    this.colorPane.position(new Vector3f(250, 100, 0)).size(new Vector3f(100, 100, 0));
    this.ui.add(this.colorPane);

    this.button = new UIButton();
    this.button.position(new Vector3f(100, 100, 0)).size(new Vector3f(100, 100, 0));
    this.button.fillColor(0xFF0000FF);
    this.button.borderColor(0x00FF00FF);
    this.button.borderWidth(10f);
  ...
  public void updateState(float deltaTime) {
    if(this.button != null) {
      //this.button.rotateZ(Math.toRadians(5.0f * deltaTime));
    }
  }

  @EventHandler
  private void onKeyDown(KeyboardEvent event) {
    if (event.key == GLFW.GLFW_KEY_ENTER && event.action == KeyboardEvent.KeyAction.PRESS) {
