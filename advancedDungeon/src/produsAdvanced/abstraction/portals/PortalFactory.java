 ...
 *
 * <p>Portals are created in pairs (blue and green) and allow entities
 * to teleport from one to the other. The factory handles portal
 * creation, positioning, collision behavior, projectile redirection,
 * and cleanup.</p>
 *
 * <p>Only one blue and one green portal can exist at a time. If a new
 * portal is created at the same position as the other, the old one
 * is cleared automatically.</p>
 */
...
=======
   ...
   *
   * <p>If a portal of the same color already exists, it will be replaced.
   * If the new portal overlaps with the other portal, the other portal
   * will be removed.</p>
   *
...
>>>>>>> c71f73d8 (decluttered portal create method)
   ...
   *
   * <p>If a portal of the same color already exists, it will be removed.
   * If the new portal would overlap with the other portal, that portal
   * will also be cleared before creation.</p>
   *
  ...
  /**
   * Sets the portal to be ignored by all {@link ProjectileSkill} so it doesn't trigger the projectiles onCollide.
   *
   * @param portal
   */
   ...
   *
   * <p>The closest floor tile to the portal is used to decide its orientation.
   * The calculated direction is also stored for teleportation logic.</p>
   *
...
>>>>>>> 24b937b6 (implemented a basic projectile teleportation)
      ...
      pc.position(bluePortal.fetch(PositionComponent.class).get().position().translate(bluePortalDirection.scale(1.2)));
...
>>>>>>> 24b937b6 (implemented a basic projectile teleportation)
      PositionComponent pc = other.fetch(PositionComponent.class).get();
      pc.position(greenPortal.fetch(PositionComponent.class).get().position().translate(greenPortalDirection.scale(1.2)));
...
>>>>>>> 20f3a7f9 (restructed portal related files):advancedDungeon/src/produsAdvanced/abstraction/portals/PortalFactory.java
  ...
  /**
   * Adjusts a projectile's velocity and goal when it passes through a portal,
   * preserving direction relative to portal orientation.
   *
  ...
  public static void handleProjectiles(Entity projectile, Direction entry, Direction exit) {
    ...
    Vector2 velocity = rotateVelocityThroughPortals(velocityComponent.currentVelocity(), entry, exit);
    Vector2 goal = projectileComponent.goalLocation().vectorTo(positionComponent.position()).rotateDeg(velocity.angleDeg());
    projectile.add(new ProjectileComponent(
      positionComponent.position(), new Point(goal.x(),goal.y()),
      projectileComponent.forceToApply(), projectileComponent.onEndReached()));
...
>>>>>>> 25d26bca (added projectile handling)
  ...
  /**
   * Creates a collision component for the portal, with bounds adjusted
   * based on its facing direction.
   *
  ...
  private static CollideComponent setCollideComponent(Direction dir, TriConsumer<Entity, Entity, Direction> onCollideEnter) {
    ...
    switch (dir){
      case DOWN -> {
        return new CollideComponent(Vector2.of(offsetX,offsetX), Vector2.of(hitboxX,hitboxY), onCollideEnter, CollideComponent.DEFAULT_COLLIDER);
      }
      case UP -> {
        return new CollideComponent(Vector2.of(offsetX,offsetY), Vector2.of(hitboxX,hitboxY), onCollideEnter, CollideComponent.DEFAULT_COLLIDER);
      }
      case LEFT-> {
        return new CollideComponent(Vector2.of(offsetX,offsetX), Vector2.of(hitboxY,hitboxX), onCollideEnter, CollideComponent.DEFAULT_COLLIDER);
      }
      case RIGHT -> {
        return new CollideComponent(Vector2.of(offsetY,offsetX), Vector2.of(hitboxY,hitboxX), onCollideEnter, CollideComponent.DEFAULT_COLLIDER);
      }
      default -> {
...
>>>>>>> efe893f0 (added PortalComponent to avoid unwanted portal on portal interactions)
   ...
   *
   * @return an {@link Optional} containing the blue portal entity,
   ...
   *
   * @return an {@link Optional} containing the green portal entity,
   *         or empty if none exists
   */
