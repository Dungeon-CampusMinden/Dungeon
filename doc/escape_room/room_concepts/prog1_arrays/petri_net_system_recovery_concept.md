...
Es beantwortet genau eine Frage: **Welcher Schritt ist als Nächstes zu lösen?** Türen, Items,
Animationen und fachliche Codeprüfungen bleiben in den jeweiligen Rätselklassen.
...
liegt genau ein Token auf genau einem Place. Der aktive Place enthält außerdem ein
`HintComponent` mit vier Hinweisen: Orientierung, Ansatz, beinahe Lösung und Lösung.
...
| 4 Transport | `TRANSPORT_ARRAY` -> `TRANSPORT_COLLECT_LOOP` -> `DATA_STORAGE_DOOR_OPEN` | Terminalschritte, danach abgeschlossener Transportlauf |
...
5. Hinweise gehören immer zum aktuell markierten Place.
