...
public class ArrayIterateLevel extends BlocklyLevel {
  ...
  private final Point doorPosition = new Point(18,3);// Das erwartete Array
   ...
   private final int[] arrayToPass = {15, 5, 9, 7, 8}; // Summe = 44
    ...
    public ArrayIterateLevel(LevelElement[][] layout, DesignLabel designLabel, List<Coordinate> customPoints) {
  ...
  @Override
    protected void onFirstTick() {
        ...
        spawnSign(signPosition, "Um diese Tür zu öffnen brauche ich die Summe eines Arrays von dir! Gehe in den Code und Implementiere die Methode in der Klasse MyArraySummarizer.");
    }
    ...
    private void checkPlayerSolution() {
        ...
        try {
            playerSolution = ((ArraySummarizer) DynamicCompiler.loadUserInstance(
              ARRAY_SUMMARIZER_PATH,
              ARRAY_SUMMARIZER_CLASSNAME,
              new Tuple<>(int[].class, arrayToPass))).summarizeArray();
      ...
      if (playerSolution == correctSolution) {
            ...
            DialogUtils.showTextPopup(
                    "Sehr gut! Du hast die korrekte Summe zurück gegeben! Der Weg ist nun frei.",
                    "Erfolg"
            );
        } else {
            DialogUtils.showTextPopup(
                    "Deine Summe ist leider nicht korrekt. Versuche es nochmal!",
                    "Fehler"
            );
        }
    }
