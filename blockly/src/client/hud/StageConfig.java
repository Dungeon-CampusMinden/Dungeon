          ...
          btn1x.addListener(
            new ChangeListener() {
              ...
              public void changed(ChangeEvent event, Actor actor) {
                if (btn1x.isChecked()) Client.MOVEMENT_FORCE = Vector2.of(baseForce.x(), baseForce.y());
              }
            });
          btn4x.addListener(
            new ChangeListener() {
              @Override
              public void changed(ChangeEvent event, Actor actor) {
                if (btn4x.isChecked()) Client.MOVEMENT_FORCE = Vector2.of(baseForce.x() * 4f, baseForce.y() * 4f);
              }
            });
          btn8x.addListener(
            new ChangeListener() {
              @Override
              public void changed(ChangeEvent event, Actor actor) {
                if (btn8x.isChecked()) Client.MOVEMENT_FORCE = Vector2.of(baseForce.x() * 8f, baseForce.y() * 8f);
              }
            });
