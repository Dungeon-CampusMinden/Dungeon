...
public class ScopedSymbolTable<KeyT, ValueT> {
  ...
  private final @NotNull Deque<
          @NotNull Pair<@NotNull Boolean, @NotNull Map<@NotNull KeyT, @NotNull ValueT>>>
   ...
   */
  private final @Nullable ScopedSymbolTable<KeyT, ValueT> parent;

  protected ScopedSymbolTable() {
    this.parent = null;
  }

  protected ScopedSymbolTable(@NotNull ScopedSymbolTable<KeyT, ValueT> parent) {
    this.parent = parent;
  }

  public static <KeyT, ValueT> ScopedSymbolTable<KeyT, ValueT> createRoot() {
    return new ScopedSymbolTable<>();
  }

  public ScopedSymbolTable<KeyT, ValueT> openNewTable() {
  ...
  public @NotNull Pair<@NotNull Boolean, @NotNull Map<@NotNull KeyT, @NotNull ValueT>> popScope() {
    assert !scopedSymbols.isEmpty() : "Cannot pop from an empty scoped symbol table.";
    return scopedSymbols.pop();
  }

  public void insertScoped(@NotNull KeyT key, @NotNull ValueT symbol) {
    ...
    scopedSymbols.peek().b.put(key, symbol);
  }

  public @NotNull Optional<ValueT> lookupScoped(@NotNull KeyT name) {
    for (var scope : scopedSymbols) {
      if (scope.b.containsKey(name)) {
        ValueT symbol = scope.b.get(name);
