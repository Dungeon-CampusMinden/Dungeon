 ...
 * Tracks, for each block, the set of values that are definitely defined on all incoming paths.
 */
public final class ReachingDefinitions {
   ...
   * <p>
   * Algorithm sketch (non-SSA):
   * <ul>
   *   <li>Lattice element: set of {@link Value} instances that are known to be defined on all paths.</li>
   *   <li>Direction: forward; Meet: intersection across predecessors (must analysis).</li>
   *   <li>Transfer: start from IN, add each op's output to the running set.</li>
   *   <li>Nesting: for each child {@link Region}, recurse with the current state as entry facts.</li>
   * </ul>
  ...
  private static void analyzeRegion(Region region, Set<Value> incoming, List<MissingDefinition> problems) {
    ...
    // The outputs of each block. These are filled as we process blocks since they can also contain values defined in
    // previous blocks, such as the entry block.
    Map<Block, Set<Value>> out = new HashMap<>();

    // The list of blocks that still needs to be processed. Start with all blocks, since we don't know which ones are
    ...
    while (!worklist.isEmpty()) {
      ...
      // (Note: we don't need to update predecessors since they can't see the new facts anyway, and the worklist will ensure we revisit successors.)
    ...
    for (Block block : blocks) {
      // The IN set for this block, which is the set of values defined on all paths to the block. Start with the merged IN set from predecessors.
      Set<Value> state = new HashSet<>(in.getOrDefault(block, incoming));
      // Walk operations in order, checking operands against the current state and updating the state with new defs.
      for (Operation op : block.getOperations()) {
        ...
        // At this point we checked all the operands and can now check the nested regions of this operation.
        // This allows us to have more precise information about the reaching definitions for the nested regions since
        // we will have already validated the operands of the parent operation and can use that information as part of
  ...
  private static Set<Value> computeIn(Block block,
    ...
    // Must-analysis: intersect the OUT sets of all predecessors. Start with the first predecessor's OUT as the initial set,
