...
public sealed interface AttributeDetails {
   ...
   * checked first; if not found, the unregistered cache is consulted, and a new {@link
   * Unregistered} placeholder is created and cached if this is the first reference to that class.
   *
   * <p><strong>Note:</strong> this method has the side effect of populating the global {@link
   * DGIRContext} caches when a new placeholder is created.
   *
   ...
   * #create(Attribute)} and registered into the global {@link DGIRContext} caches by {@link
  ...
  record Registered(
    ...
    /**
     * Register the given attribute prototype into the global {@link DGIRContext} caches. If the
     * attribute already carries a {@link Registered} details instance it is reused; otherwise
   ...
   *
   * <p>The {@code type} field is always set to {@link Attribute Attribute.class} because the
   * concrete subclass is not yet known. The {@code dialectOpt} field is populated by {@link
   ...
   * resolved) are usable on this placeholder; all other accessors throw {@link
