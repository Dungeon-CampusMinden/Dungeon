...
public class PrintRunner extends OpRunner {
  ...
  public PrintRunner() {
    super(RegisteredOperationDetails.get(PrintOp.class));
  }

  @Override
  protected @NotNull Action runImpl(@NotNull Operation op, @NotNull State state) {
    PrintOp printOp = op.as(PrintOp.class);
    assert !printOp.getOperands().isEmpty() : "Print operation must have at least one operand";

    if (printOp.getOperands().size() == 1) {
      out.print(state.getValue(printOp.getOperands().getFirst().getValue()));
    } else {
      Object formatString = state.getValue(printOp.getOperands().getFirst().getValue());
