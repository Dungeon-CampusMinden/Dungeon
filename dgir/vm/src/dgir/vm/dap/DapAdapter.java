 ...
 *
 * <p>Implements both {@link IDebugProtocolServer} (lsp4j's generated DAP server interface, wired
 * to the client over a socket by {@link DapServer}) and {@link Debugger} (the VM's debug
 * callback interface).
 *
 * <h2>Lifecycle</h2>
 * <ol>
 *   <li>{@link DapServer} creates an instance per client connection and passes it to
 *       {@code DSPLauncher.createServerLauncher()}.
 *   <li>lsp4j calls {@link #initialize} / {@link #launch} or {@link #attach} / breakpoint
 *       requests / {@link #configurationDone}.
 *   <li>After {@code configurationDone} the VM runs on a daemon thread, invoking
 *       {@link #onStep} and {@link #onBreakpointHit} before each operation.
 *   <li>Those callbacks fire {@code stopped} events back to the client and block until
 *       {@link #continue_(ContinueArguments)} or {@link #next(NextArguments)} unblocks them.
 * </ol>
 */
public class DapAdapter implements IDebugProtocolServer, Debugger {
   ...
   * Cleared and used by {@link #onStep} to fire a {@code "step"} stopped event instead of
   * a generic {@code "pause"} event after the step completes.
   */
  ...
  public CompletableFuture<StackTraceResponse> stackTrace(StackTraceArguments args) {
    ...
    for (int i = 0; i < callStack.size(); i++) {
      StackFrame frame = getStackFrame(callStack, i);
      frames.add(frame);
    }
  ...
  private static @NotNull StackFrame getStackFrame(@NotNull List<Operation> callStack, int i) {
    ...
    frame.setId(i + 1);
    frame.setName(loc.equals(Location.UNKNOWN)
        ? op.getDetails().ident()
        : loc.toString());
    frame.setSource(src);
  ...
  @Override
  public @NotNull DebugControl onStep(
      @NotNull Operation operation, @NotNull Location location) {
  ...
  /**
   * Launch the VM on a daemon thread. Waits for {@link #configDone} before calling
   * {@link VM#run()} so all breakpoints set during client startup are already registered.
   */
