...
public class Shader implements Disposable {
  ...
  private static final String compileErrorRegex =
      "^(?<file>[0-9]+)\\((?<line>[0-9]+)\\)(?<message>.*)$";
  private static final Pattern compileErrorPattern =
      Pattern.compile(compileErrorRegex, Pattern.MULTILINE);
   ...
   *
   * @param resource The resource containing the shader code.
   * @param alreadyIncluded A set of resources that have already been included to prevent circular
   *     includes.
   * @return The parsed shader source code.
   * @throws IOException If an I/O error occurs while reading the shader code from the resource.
   */
  private static String parseShaderSource(Resource resource, Set<Resource> alreadyIncluded)
      throws IOException {
  ...
  private void initGL() {
    ...
    if (GL33.glGetShaderi(this.glHandle, GL33.GL_COMPILE_STATUS) == GL33.GL_FALSE) {
      ...
      while (matcher.find()) {
        errorOutput
            .append("File: ")
            .append(matcher.group("file"))
            .append(", Line: ")
            .append(matcher.group("line"))
            .append(", Message: ")
            .append(matcher.group("message"))
            .append("\n");
      }
