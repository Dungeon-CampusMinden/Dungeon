 ...
 * feature.hud.dialogs.DialogCallbackResolver} when the user clicks a button, submits input, or
 * closes a dialog.
 *
 * <p>The server validates this message against {@link engine.network.server.DialogTracker} to
 * ensure the client is authorized to respond and, for shared dialogs, only the first response
 * triggers a callback.
 *
 ...
 * @see engine.network.messages.s2c.DialogShowMessage
 * @see engine.network.server.DialogTracker
 */
public record DialogResponseMessage(String dialogId, String callbackKey, Payload payload)
  ...
  /**
   * Wraps a custom value decoded by a {@link engine.network.codec.DialogValueCodec} that does not
