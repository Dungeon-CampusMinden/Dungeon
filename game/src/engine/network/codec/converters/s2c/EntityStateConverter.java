...
public final class EntityStateConverter
    implements MessageConverter<EntityState, engine.network.proto.s2c.EntityState> {
  private static final byte WIRE_TYPE_ID = 14;

  @Override
  public engine.network.proto.s2c.EntityState toProto(EntityState message) {
    engine.network.proto.s2c.EntityState.Builder builder =
        engine.network.proto.s2c.EntityState.newBuilder().setEntityId(message.entityId());
    ...
    if (hasPosition || hasViewDirection || hasRotation || hasScale) {
      engine.network.proto.common.PositionInfo.Builder positionInfo =
          engine.network.proto.common.PositionInfo.newBuilder();
        ...
        .ifPresent(
            slots -> {
              for (InventorySlotState inventorySlot : slots) {
                engine.network.proto.s2c.ItemSlot.Builder slot =
                    engine.network.proto.s2c.ItemSlot.newBuilder()
  ...
  @Override
  public EntityState fromProto(engine.network.proto.s2c.EntityState proto) {
    ...
    if (proto.hasPosition()) {
      engine.network.proto.common.PositionInfo positionInfo = proto.getPosition();
      ...
      if (positionInfo.hasViewDirection()) {
        engine.network.proto.common.Direction viewDirection = positionInfo.getViewDirection();
        if (viewDirection != engine.network.proto.common.Direction.DIRECTION_UNSPECIFIED
            && viewDirection != engine.network.proto.common.Direction.UNRECOGNIZED) {
    ...
    List<engine.network.proto.s2c.ItemSlot> slots = proto.getInventoryList();
    if (!slots.isEmpty()) {
      List<InventorySlotState> inventorySlots = new ArrayList<>(slots.size());
      for (engine.network.proto.s2c.ItemSlot slot : slots) {
  ...
  @Override
  public Class<engine.network.proto.s2c.EntityState> protoType() {
    return engine.network.proto.s2c.EntityState.class;
  }

  @Override
  public Parser<engine.network.proto.s2c.EntityState> parser() {
    return engine.network.proto.s2c.EntityState.parser();
  }
