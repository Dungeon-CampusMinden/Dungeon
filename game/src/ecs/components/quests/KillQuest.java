...
public class KillQuest extends Quest {
    ...
    public static boolean bossQuestExists = false;
    ...
    /**
     * If no boss kill ques exists an new boss kill quest will be created
     * otherwise it will call {@code buildKillQuest()}
     * 
     * @param questHolder entity that owns the quest
     * @return if no boss kill ques exists return a new boss kill quest
     *         <p>
     *         otherwise return a new kill quest
     */
    public static KillQuest buildBossKillQuest(Entity questHolder) {
        if (bossQuestExists)
            return buildKillQuest(Monster.class, questHolder);
        String name = "Dungeon Master";
        int total = 1;
        ITask task = new ITask() {
            ...
            public void advance() {
                count += Game.getEntities().stream()
                        // Consider only entities that are Bosses
                        .filter(e -> Boss.class.isAssignableFrom(e.getClass()))
                        // Consider only entities that have a HealthComponent
                        .flatMap(e -> e.getComponent(HealthComponent.class).stream())
                        // Form triples (e, hc, ac)
                        .map(hc -> buildDataObject((HealthComponent) hc))
        ...
        IReward reward = RewardBuilder.buildBossReward();
        String description = "Kill the Boss to " + reward.toString() + ".";
        bossQuestExists = true;
        return new KillQuest(name, description, task, reward, questHolder);
    }
