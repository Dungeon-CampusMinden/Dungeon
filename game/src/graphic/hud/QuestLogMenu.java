...
public class QuestLogMenu<T extends Actor> extends ScreenController<T> {
    ...
    public QuestLogMenu(SpriteBatch batch) {
        super(batch);
        hideMenu();
    }
     ...
     */
    public void display(String text, int questAmount) {
        if (screenText != null)
            remove((T) screenText);
        ...
        screenText.setPosition(
                ...
                (Constants.WINDOW_HEIGHT) * 7f / 10f - 100 * questAmount,
                Align.center | Align.bottom);
        add((T) screenText);
        LOGGER.info("Questlog displayed");
        showMenu();
    }
}
