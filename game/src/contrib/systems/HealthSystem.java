...
public final class HealthSystem extends System {
    ...
    private HSData activateDeathAnimation(HSData hsd) {
        ...
        hsd.dc.queueAnimation(
                hsd.dc.getAnimation(AdditionalAnimations.DIE).get().duration(),
                AdditionalAnimations.DIE);
        return hsd;
    }
    ...
    private void doDamageAndAnimation(HSData hsd, int dmgAmount) {
        if (dmgAmount > 0) {
            ...
            hsd.dc.queueAnimation(
                    hsd.dc.getAnimation(AdditionalAnimations.HIT).get().duration(),
                    AdditionalAnimations.HIT);
        }
