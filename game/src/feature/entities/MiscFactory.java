...
import feature.components.CatapultableComponent;
...
import feature.inventory.components.ItemGenerator;
import feature.inventory.items.ItemBigKey;
import feature.inventory.items.ItemFairy;
import feature.inventory.items.ItemHammer;
import feature.inventory.items.ItemHeart;
...
public final class MiscFactory {
  ...
  public static Entity cookingPot(Point position, int inventorysize, Item... items) {
    ...
    Arrays.stream(items).forEach(ic::add);
    cookingPot.add(ic);
    cookingPot.add(
        new InteractionComponent(
            new Interaction(
  ...
  private static void catapultFlyEntity(Entity other, Point start, Point goal, float speed) {
    ...
    other
        ...
        .ifPresentOrElse(pc -> pc.position(start), () -> other.add(new PositionComponent(start)));
