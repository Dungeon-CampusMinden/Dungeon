...
public abstract class CursorSkill extends Skill {
   ...
   *
   * <p>By default, this uses {@link CursorUtils#positionInWorld()}, but can be overridden using
   * {@link #cursorPositionSupplier(Supplier)} for flexibility in different contexts (e.g., testing
   * or server-side logic).
   */
  private Supplier<Point> cursorPositionSupplier = CursorUtils::positionInWorld;
   ...
   * CursorUtils#positionInWorld()} or a custom supplier if overridden, and delegates the effect to
   * {@link #executeOnCursor(Entity, Point)}.
   *
