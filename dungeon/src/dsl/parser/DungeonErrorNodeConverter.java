...
public class DungeonErrorNodeConverter implements DungeonDSLParserVisitor<Node> {
  ...
  public Node visitProgram(DungeonDSLParser.ProgramContext ctx) {
    var node = new Node(Node.Type.Program, this.childNodes);
    node.setSourceFileReference(ctx);
    return node;
  }
  ...
  public Node visitStmt(DungeonDSLParser.StmtContext ctx) {
    var node = new Node(Node.Type.Stmt, this.childNodes);
    node.setSourceFileReference(ctx);
    return node;
  }
  ...
  public Node visitChildren(RuleNode ruleNode) {
    ...
    if (ruleNode instanceof DungeonDSLParser.Import_defContext ctx) {
      var node = new ImportNode();
      node.addChildren(this.childNodes);
      node.setSourceFileReference(ctx);
      return node;
    } else if (ruleNode instanceof DungeonDSLParser.Property_defContext ctx) {
      var node = new PropertyDefNode();
      node.addChildren(this.childNodes);
      node.setSourceFileReference(ctx);
      return node;
    } else if (ruleNode instanceof DungeonDSLParser.Param_defContext ctx) {
      var node = new ParamDefNode();
      node.addChildren(this.childNodes);
      node.setSourceFileReference(ctx);
      return node;
    } else if (ruleNode instanceof DungeonDSLParser.Type_declContext ctx) {
      var node = new Node(Node.Type.TypeSpecifier);
      node.addChildren(this.childNodes);
      node.setSourceFileReference(ctx);
      return node;
    }
