...
/**
 * This class contains static methods to create the TaskSelector-Level to select the tasks at the start of the game.
 *
...
public class TaskSelector {
    ...
    protected static TaskConfiguration selectedNewDSLEntryPoint = null;
    ...
    protected static SingleChoice selectNewDSLTaskQuestion(Set<TaskConfiguration> entryPoints) {
        SingleChoice question = new SingleChoice("Wähle deine Mission:");
        entryPoints.forEach(ep -> question.addAnswer(new NewDSLPayloadTaskContent(ep)));
        question.state(Task.TaskState.PROCESSING_ACTIVE);
        question.taskName(" ");
        return question;
    }
    ...
    protected static Entity newDSLNpc(SingleChoice selectionQuestion) throws IOException {
        Entity npc = new Entity("Selection NPC");
        npc.add(new DrawComponent(new SimpleIPath("character/blue_knight")));
        npc.add(new PositionComponent());
        npc.add(
            new InteractionComponent(
                1,
                true,
                UIAnswerCallback.askOnInteraction(selectionQuestion, setSelectedNewDSLEntryPoint())));
    ...
    private static BiConsumer<Task, Set<TaskContent>> setSelectedNewDSLEntryPoint() {
        return (task, taskContents) -> {
            if (taskContents.isEmpty()) return;
            selectedNewDSLEntryPoint =
                ((NewDSLPayloadTaskContent)
                    taskContents.stream()
                        .findFirst()
                        .orElseThrow(
                            () ->
    ...
    private static class NewDSLPayloadTaskContent extends Quiz.Content {
        private final TaskConfiguration payload;

        public NewDSLPayloadTaskContent(TaskConfiguration payload) {
            super(payload.getId());
            this.payload = payload;
        }

        public TaskConfiguration payload() {
            return payload;
        }
    }
}
