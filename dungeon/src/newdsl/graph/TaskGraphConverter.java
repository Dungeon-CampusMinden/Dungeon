...
import newdsl.graph.petrinet.PetriNet;
import newdsl.graph.petrinet.PetriNetFactory;
...
public class TaskGraphConverter {
    ...
    public static void callTaskBuilderFor(
        ...
        graph
            ...
            .forEachRemaining(
                taskNode ->
                    interpreter
                        .buildTask(taskNode.getTask())
                        .ifPresent(
                            buildTask -> taskNode.getTask().setEntitySets((Set<Set<Entity>>) buildTask)));
    }
     ...
     * TaskGraph-Edges. The doors between the graphs will be closed, and a {@link DoorComponent} for the manager Entity
     * of the Task to which the door leads will be created, so the entity can open the door if it gets activated.
     *
    ...
    public static ILevel levelGraphFor(final TaskDependencyGraph taskGraph) {
        ...
        taskGraph
            ...
            .forEachRemaining(
                node -> {
                    LevelGraph levelGraph = LevelGraphGenerator.generate(node.getTask().getEntitySets());
                    nodeToLevelGraph.put(node, levelGraph);
                    graphToTask.put(levelGraph, node.getTask());
                });
        ...
        taskGraph
            ...
            .forEachRemaining(
                edge -> {
                    TaskNode start = edge.getStartNode();
                    TaskNode end = edge.getEndNode();
     ...
     * the root graph are connected to any of the level graphs in the collection. If not, it adds the first unconnected
     * level graph to the root graph and recursively continues to connect unconnected level graphs until all are
     * connected.
     *
     ...
     * @return {@code true} if all level graphs are connected to the root graph, {@code false} otherwise (never, return
     * value is used for the recursion).
     */
    ...
    /**
     * Connects each door that connects two level graphs to the manager entity responsible for the respective graph
     * part.
     *
     ...
     *
     * @param levelGraphToTask Mapping of the Level-graphs to the Tasks, so this function knows which Task is associated
     *                         with which level graph.
     */
    private static void connectDoorsWithTaskManager(Map<LevelGraph, Task> levelGraphToTask) {
        ...
        taskToDoor.forEach(
            (task, doorTiles) -> {
                ...
                task.getManagementEntity()
                    .ifPresentOrElse(
                        entity -> ((Entity) entity).add(doorComponent),
                        () -> {
                            Entity manager = new Entity();
                            manager.add(new NewDSLTaskComponent(task, manager));
    ...
    public static Map<TaskNode, PetriNet> petriNetFor(final TaskDependencyGraph taskGraph) {
        ...
        taskGraph
            .nodeIterator()
            .forEachRemaining(
                taskNode -> noteToNet.put(taskNode, PetriNetFactory.defaultNet(taskNode.getTask())));
            ...
            .forEachRemaining(
                taskEdge ->
                    PetriNetFactory.connect(
                        noteToNet.get(taskEdge.getEndNode()),
                        noteToNet.get(taskEdge.getStartNode()),
                        taskEdge.getEdgeType()));
