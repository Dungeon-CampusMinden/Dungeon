...
public class DSLEntryPointFinder implements AstVisitor<Object> {
  private ArrayList<entrypoint.DSLEntryPoint> entryPoints;
  private entrypoint.ParsedFile parsedFile;
  private final GameEnvironment environment;
  private AggregateType questConfigDataType;
  private final HashMap<Path, entrypoint.ParsedFile> parsedFiles;

  /**
   * Constructor.
   *
   * <p>Creates a GameEnvironment to get the {@link IType} for {@link entrypoint.DungeonConfig}.
   */
   ...
   * and creates {@link entrypoint.DSLEntryPoint} instances for each one.
   *
   ...
   */
  public Optional<List<entrypoint.DSLEntryPoint>> getEntryPoints(Path filePath) {
    Node programAST;
    if (this.parsedFiles.containsKey(filePath)) {
      ...
      entrypoint.ParsedFile parsedFile = new ParsedFile(filePath, programAST);
    ...
    List<entrypoint.DSLEntryPoint> list = findEntryPoints(programAST);
    if (list.size() != 0) {
  ...
  private List<entrypoint.DSLEntryPoint> findEntryPoints(Node programAST) {
