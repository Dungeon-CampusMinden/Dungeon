...
/**
 * The ShineOverlayRenderer class provides rendering methods for generating
 * animated shine overlay effects on an input image.
 *
 * <p>The implementation supports two rendering strategies: single-sweep overlays for small sprites and repeated
 * slice overlays for larger images.
 *
 * <p>The rendered overlay is affected by various
 * parameters such as padding, slice count, gap size, rotation speed, and shine color,
 * provided through a {@code ShineRenderConfig} configuration object.
 *
 * <p>The rendering process adapts intensity, transparency, and radial fade effects
 * based on the mask alpha, geometry, and time-dependent animations. The resulting
 * output is a padded {@code BufferedImage} with added shine overlay effects.
 *
...
final class ShineOverlayRenderer {
  ...
  static BufferedImage render(
    ...
    return renderRepeatedSlicesOverlay(
      paddedWidth, paddedHeight, alphaMask, config, nowSeconds);
  }
  ...
  private static BufferedImage renderRepeatedSlicesOverlay(
    ...
    final float sweepPhase =
      (float) normalizePhase(nowSeconds * config.rotationSpeed() * 1.8);
    final float pulse =
      0.80f
        + 0.20f
        * (0.5f
        + 0.5f
        * (float) Math.sin(nowSeconds * config.rotationSpeed() * TWO_PI));
    final Color shineColor = config.shineColor();
    final float shineAlpha = shineColor.getAlpha() / 255.0f;
