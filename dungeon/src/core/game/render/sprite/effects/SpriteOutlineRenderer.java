 ...
 *
 * <p>SpriteOutlineRenderer provides static methods for applying visual effects to sprites and
 * images, such as outlines, tinting, and color animations. It includes caching mechanisms for
 * performance optimization when rendering frequently used effects.
 *
 * <p>Key features:
 *
 * <ul>
...
public final class SpriteOutlineRenderer {
   ...
   * given color and thickness. The outline is created by drawing multiple offset copies of a tinted
   * silhouette behind the original sprite.
   *
   ...
   *
   * <p>If beat intensity is set, the outline width oscillates sinusoidally with time, creating a
   * pulsing effect. The oscillation frequency is controlled by the beat speed parameter.
   *
   ...
   */
  public static int effectiveOutlineWidth(final SpriteOutlineComponent effect, final long nowMs) {
   ...
   */
  public static Color effectiveOutlineColor(final SpriteOutlineComponent effect, final long nowMs) {
    if (!effect.rainbow()) {
      return effect.color();
    }
    ...
    return new Color(
        rainbow.getRed(), rainbow.getGreen(), rainbow.getBlue(), effect.color().getAlpha());
  }
