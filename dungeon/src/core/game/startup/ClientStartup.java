 ...
 *
 * <p>This utility class manages the one-time initialization of the client, including user setup,
 * networking startup, and initial level loading. All initialization is performed exactly once, even
 * if the setup method is called multiple times.
 */
public final class ClientStartup {
   ...
   *
   * <ul>
   *   <li>1. Executes user setup (spawning hero and other initial entities)
   *   <li>2. Starts networking (might be a LocalNetworkHandler for Single Player)
   *   <li>3. Executes all configured client startup tasks
   *   <li>4. Loads the initial level
   * </ul>
   *
   * <p>Subsequent calls to this method will have no effect. If an exception occurs during setup, it
