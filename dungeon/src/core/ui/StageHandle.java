 ...
 * framework-agnostic UI operations such as adding actors, managing focus, and querying
 * stage properties.
 *
 * <p>Key capabilities:
 * <ul>
...
public interface StageHandle {
   ...
   *
   * <p>This provides access to the framework-specific stage implementation for cases where
   * direct access is necessary.
   *
   ...
   *
   * <p>This method attempts to cast the underlying stage to the specified type and returns
   * an Optional containing it if successful, or an empty Optional if the underlying object
   ...
   *
   * <p>The actor will be rendered and receive input events according to the stage's
   * event handling and rendering order.
   *
   ...
   *
   * <p>The actor will receive keyboard input events while it has focus. Only one actor
