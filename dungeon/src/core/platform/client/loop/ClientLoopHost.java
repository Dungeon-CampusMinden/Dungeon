 ...
 *
 * <p>The {@code ClientLoopHost} manages initialization, configuration, and runtime systems required
 * for running a game client. This includes setting up rendering, input, sound playback, and other
 * platform-specific resources.
 *
 * <p>It integrates the game loop into the client runtime environment. Optional gameplay, debugging,
 * or user interface systems can be supplied explicitly through {@link ClientLoopHostInstaller}
 * implementations.
 *
 * <p>Responsibilities:
 *
 * <ul>
 *   <li>Initializes the game engine and core runtime systems.
 *   <li>Configures platform-specific services, such as rendering, input, and audio playback.
 *   <li>Manages runtime systems for HUD, gameplay extensions, and debugging.
 *   <li>Provides access to a client-side {@link StageHandle} for managing UI components.
 *   <li>Offers a configurable sound player through the {@code soundPlayer()} method.
 * </ul>
 *
...
public final class ClientLoopHost implements GameLoopHost {
  ...
  public ClientLoopHost(Collection<? extends ClientLoopHostInstaller> installers) {
    this.installers =
        List.copyOf(Objects.requireNonNull(installers, "installers must not be null"));
  }
