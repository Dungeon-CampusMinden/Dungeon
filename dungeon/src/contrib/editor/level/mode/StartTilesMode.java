 ...
 * (typically players) in the level. Each start tile is assigned a unique slot and visualized with a
...
public final class StartTilesMode extends LevelEditorMode {
  ...
  /**
   * Initializes a new instance of the StartTilesMode class, which represents a mode in the level
   * editor for defining and managing start tiles.
   *
  ...
  protected void execute() {
    int maxIndex =
        system().currentDungeonLevelForModes().map(level -> level.startTiles().size()).orElse(0);
