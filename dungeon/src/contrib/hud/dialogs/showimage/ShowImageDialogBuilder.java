 ...
 *
 * <p>It retrieves image configuration from the dialog context, including the image path, transition
 * speed, size constraints, and optional overlay text.
 */
public final class ShowImageDialogBuilder {
   ...
   *
   * <p>This method retrieves the required image path and optional configuration parameters from the
   * dialog context.
   *
   * <p>If optional parameters are not provided, sensible defaults are used:
   *
   * <ul>
   *   <li>Transition speed defaults to MEDIUM
   *   <li>The maximum size defaults to 0.85 (85% of screen)
   *   <li>Image text defaults to null (no text overlay)
   *   <li>Text scale defaults to 1.0
   *   <li>Text color defaults to black with full opacity (0x000000ff)
   * </ul>
   *
  ...
  public static UiHandle build(DialogContext ctx) {
    ...
    float maxSize = ctx.find(DialogContextKeys.IMAGE_MAX_SIZE, Float.class).orElse(0.85f);

    String imageText = ctx.find(DialogContextKeys.IMAGE_TEXT, String.class).orElse(null);

    float imageTextScale = ctx.find(DialogContextKeys.IMAGE_TEXT_SCALE, Float.class).orElse(1f);
