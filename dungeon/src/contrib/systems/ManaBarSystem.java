...
/**
 * Creates a progress bar which follows the associated Entity and shows the current mana percentage.
 * *
 *
 * <p>Entities with the {@link ManaComponent} and {@link PositionComponent} will be processed by
 * this system.
 */
public final class ManaBarSystem extends System {
  ...
  private static final float MANA_BAR_UPDATE_DURATION = 0.1f;
    ...
    this.onEntityAdd =
        (x) -> {
          ...
          ProgressBar newHealthBar =
              createNewManaBar(x.fetch(PositionComponent.class).orElseThrow());
  ...
  private void update(final EnemyData ed) {
    ...
    ed.pb.setVisible(ed.dc.isVisible() && ed.mc.getCurrentAmount() != ed.mc.getMaxAmount());
    updatePosition(ed.pb, ed.pc);

    // set value to health percent
    ed.pb.setValue((float) ed.mc.getCurrentAmount() / ed.mc.getMaxAmount());
  }
  ...
  private ProgressBar createNewManaBar(PositionComponent pc) {
    ProgressBar progressBar = new ProgressBar(MIN, MAX, STEP_SIZE, false, defaultSkin(), "manabar");
    progressBar.setAnimateDuration(MANA_BAR_UPDATE_DURATION);
  ...
  private void updatePosition(ProgressBar pb, PositionComponent pc) {
    ...
    pb.setPosition(screenPosition.x, screenPosition.y - 20);
  }
