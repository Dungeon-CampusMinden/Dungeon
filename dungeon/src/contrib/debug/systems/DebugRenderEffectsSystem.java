...
public final class DebugRenderEffectsSystem extends System {
  ...
  private static final String PASSTHROUGH_DEBUG_EFFECT_ID = "debug_passthrough_scene_effect";
  ...
  private static final int STARTER_DEPTH_COLOR_GRADE_DEMO_LAYER = DepthLayer.ForegroundDeco.depth();
   ...
   *
   * <p>This system is configured to operate on the client-side and is primarily used for debugging
   * and rendering various visual effects within the application.
   */
  ...
  public void execute() {
    ...
    if (InputManager.isKeyJustPressed(
      ...
      if (isShiftPressed()) {
        ...
        LOGGER.info("Depth-layer-pass effects are now {}.", enabled ? "enabled" : "disabled");
      }
    }
  ...
  private void togglePassthroughPmaDebug() {
    ...
    LOGGER.info("Passthrough alpha debug is now {}.", newState ? "enabled" : "disabled");
  }
  ...
  private void togglePassthroughWorldPosDebug() {
    ...
    LOGGER.info("Passthrough world-position debug is now {}.", newState ? "enabled" : "disabled");
  }
  ...
  private void toggleDemoDepthLayerEffectGroup() {
    ...
    boolean enabled = DepthLayerEffectPipeline.toggleAll(STARTER_DEPTH_COLOR_GRADE_DEMO_LAYER);
