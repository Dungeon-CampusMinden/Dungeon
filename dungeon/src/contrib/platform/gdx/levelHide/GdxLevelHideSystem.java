package contrib.platform.gdx.levelHide;
...
/**
 * libGDX-backed render system for level-hide regions.
 *
 * <p>This system manages the shader instances that visually hide and reveal configured
 * level regions. The gameplay data remains in {@link LevelHideComponent}; only the
 * concrete shader/render integration lives here.
 */
public class GdxLevelHideSystem extends System {
  ...
  /** Constructs new GdxLevelHideSystem. */
  public GdxLevelHideSystem() {
      ...
      .ifPresent(
        ds -> {
          ...
          ds.sceneShaders()
            .add(
              shaderIdentifier,
              new LevelHideShader(true, region)
                .transitionSize(data.lhc.transitionSize()));
        });
  }
      ...
      .ifPresent(
        ds -> {
          filteredEntityStream()
            .map(Data::of)
            .forEach(entity -> checkRegion(ds, entity, lastPlayerPosition, currentPos));
        });
  ...
  private void checkRegion(DrawSystem ds, Data d, Point lastPos, Point currentPos) {
    ...
    if (!(ds.sceneShaders().get(shaderIdentifier) instanceof LevelHideShader lhs)) {
      return;
    }
  ...
  private Optional<DrawSystem> getDrawSystem() {
    if (!(Game.systems().get(DrawSystem.class) instanceof DrawSystem ds)) {
