...
public class EmailsTab extends ComputerTab {
  ...
  private Table createEmailHeader(Email email) {
    ...
    Label subjectLabel =
        Scene2dElementFactory.createLabel(Localization.getInstance().getCurrentTranslator().translate(email.subject), 18, isSelected ? Color.WHITE : Color.BLACK);
  ...
  private Table createEmailDetailsContainer() {
    ...
    Label subjectLabel =
        Scene2dElementFactory.createLabel("Subject: " + Localization.getInstance().getCurrentTranslator().translate(selectedEmail.subject()), 22, Color.BLACK);
  ...
  public record Email(
    ...
    public List<String> parsedContentLines() {
      return Arrays.asList(Localization.getInstance().getCurrentTranslator().translate(content).split(PARAGRAPH_SPLIT));
    }
  }
