...
public class BrowserTab extends ComputerTab {
  ...
  protected void createActors(){
    ...
    this.add(Scene2dElementFactory.createHorizontalDivider()).height(4).pad(10, 0, 10, 0).fillX().row();
  ...
  private Table createAdressBar(){
    ...
    urlField.addListener(new InputListener() {
      @Override
    ...
    goButton.addListener(new ClickListener(Input.Buttons.LEFT) {
      @Override
      public boolean touchDown(InputEvent event, float x, float y, int pointer, int button) {
        url = urlField.getText().trim();
        navigate(url);
  ...
  private Actor create404Page(){
    Table table = new Table();
    Label label = Scene2dElementFactory.createLabel("404 - Page Not Found", 48, new Color(0.8f, 0, 0, 1));
    table.add(label);
    return table;
  }
  ...
  private Actor createNoContentPage(){
    Table table = new Table();
    Label label = Scene2dElementFactory.createLabel("Enter a URL to visit a website!", 48, Color.GRAY);
    table.add(label);
    return table;
  }
  ...
  private Actor createVirusPage(){
    ...
    Label desc = Scene2dElementFactory.createLabel("Your computer has been infected with a virus!\nPlease restart your computer to remove the threat.", 48, Color.RED);
    desc.setWrap(true);
  ...
  private void registerWebsites(){
    ...
    websites.put("https://www.example.com", Scene2dElementFactory.createLabel("Helloooo world :D", 96, Color.BLACK));
