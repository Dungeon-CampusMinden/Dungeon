...
public class BlogTab extends ComputerTab {
  ...
  protected void createActors() {
    ...
    this.add(Scene2dElementFactory.createScrollPane(container, false, true)).maxWidth(800).minWidth(500).grow();
  }
  ...
  private Table createBlogEntryTable(BlogEntry entry){
    ...
    if(!entry.comments.isEmpty()){
      ...
      table.add(Scene2dElementFactory.createHorizontalDivider()).growX().height(4).pad(20, 0, 15, 0).colspan(2).row();
      table.add(Scene2dElementFactory.createVerticalDivider()).growY().padLeft(50).padRight(10).width(4);
  ...
  private Table createCommentTable(BlogComment comment, boolean isFirst){
    Label user = Scene2dElementFactory.createLabel("\""+comment.user + "\" says:", 16, Color.BLACK);
    ...
    if(!isFirst){
      table.add(Scene2dElementFactory.createHorizontalDivider()).growX().height(4).pad(10, 0, 15, 0).row();
    }
