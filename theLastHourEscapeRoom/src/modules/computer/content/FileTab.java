...
public class FileTab extends ComputerTab {
  ...
  private Actor create404Page(String fileName){
    ...
    Label label = Scene2dElementFactory.createLabel("Could not read file '%s' :(".formatted(fileName), 48, new Color(0.8f, 0, 0, 1));
    table.add(label);
    return table;
  }
  ...
  private Actor createHelpPage(){
    ...
    List<Tuple<String, Integer>> helpEntries = List.of(
    ...
    table.add(Scene2dElementFactory.createLabel("~~~ HELP ~~~", 64, new Color(0, 0.7f, 0, 1))).row();
  ...
  private void registerFiles(){
    files = new HashMap<>();
    files.put("Hello.html", Scene2dElementFactory.createLabel("Helloooo world :D", 96, Color.BLACK));
