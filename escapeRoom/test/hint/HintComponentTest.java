 ...
 *
 * <p>Tests cover initialization, hint retrieval by index, behavior of {@link
...
public class HintComponentTest {
  ...
  void testConstructorWithValidHints() {
    ...
    assertEquals("hint1", hc.nextHint(), "Next hint should initially be the first hint");
  }
  ...
  /**
   * Tests {@link HintComponent#nextHint()} in combination with {@link
   * HintComponent#increaseIndex()}. Verifies that the next hint is returned correctly and that
   * after the last hint, nextHint() returns an empty string and {@link
   * HintComponent#isLastHintShown()} is true.
   */
  @Test
  void testNextHintAndIncreaseIndex() {
    ...
    assertEquals("first", hc.nextHint());
    assertFalse(hc.isLastHintShown());

    // Increase index and get next hint
    hc.increaseIndex();
    assertEquals("second", hc.nextHint());
    assertFalse(hc.isLastHintShown());

    // Increase index beyond last hint
    hc.increaseIndex();
    assertEquals("", hc.nextHint(), "Next hint should be empty after last hint");
    assertTrue(hc.isLastHintShown());
  }

  /**
   * Tests multiple calls to {@link HintComponent#increaseIndex()} to ensure that the internal index
   * does not break and {@link HintComponent#nextHint()} returns an empty string after exceeding the
   * last hint.
   */
  @Test
